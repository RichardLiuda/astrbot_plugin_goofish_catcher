...
当前插件版本（v4.0.1）需要 Worker **≥ v3.5.0**。
...
### Fixed（自然语言 / 斜杠命令订阅淘宝被默认 AI 抢走）

- 明确的订阅句（「订阅淘宝的 总统黄油」、带「」引号、`/闲鱼 订阅淘宝的 …` 粘连写法）改为确定性解析并直接建订阅，不再落到 AstrBot 默认 LLM 闲聊。
- `/闲鱼 订阅` 关键词改为整段接收，支持 `淘宝 总统黄油` / `淘宝的 总统黄油` 及带空格关键词；无法识别的 `/闲鱼 …` 回用法说明而不是交给闲聊 AI。
- `/闲鱼 退订` / `暂停` / `恢复` / `立即检查` 同样识别可选平台前缀，避免刚建的淘宝订阅按闲鱼关键词找不到。
- 意图引擎平台预提取补上「订阅/监控/蹲淘宝的」形态，避免平台词留在关键词里。

## [4.0.0] - 2026-07-27

多平台（淘宝）支持全量落地：订阅监控、采购决策比价、扫码登录与登录态管理、
实时搜索、详情深度分析均支持 `platform="taobao"`，体验与数据管理对齐闲鱼。
包含 PR #6（多平台采购决策）合入后的全部改造与本次登录/搜索链路对齐。
