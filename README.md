...
「淘宝的 RTX5090 帮我蹲一个」「闲鱼淘宝比比价，哪个值得买」——双平台订阅监控、自然语言采购决策比价、扫码登录与登录态管理全对齐。

🤖 v3.0.0 起全面 Agent 化：直接对 LLM 说需求，自动调工具、驱动真实浏览器、进度实时推流到对话，即问即用。

<br>
</div>

> [!CAUTION]
> **远程模式用户请务必同步更新 Worker 代码。** v3.0.0 新增了 Agent 远程执行接口，旧版 Worker 与新版插件不兼容。淘宝功能暂不支持远程模式（仅 `playwright_local`）。
...
- **多平台（v4.0.0）**：闲鱼 + 淘宝双平台订阅监控；扫码登录、登录态管理、实时搜索、详情深度分析全平台对齐（详见下方[多平台支持](#多平台支持淘宝)）
- **采购决策比价（v4.0.0）**：`buyagent_purchase_decision`——自然语言需求 → 意图拆解（关键词/预算/成色）→ 多平台并发搜索 → 找不到自动逐级放宽 → 聚合排序输出决策卡片；需求里点名的平台（"淘宝的 xxx"）自动识别并只搜点名平台
...
- 支持统一补登录态：二维码截图下发、扫码后回复任意消息继续、记住 cookie 免扫码自动登录、自动恢复暂停订阅
- 支持 WebUI 管理与运行状态查看
- **浏览器 Agent**：LLM 可调用 `goofish_browser_task` 驱动真实 Chromium 执行复杂页面任务；进度实时推流到对话界面；远程模式下 Agent 运行在 Worker，本地无需 Playwright
- **`goofish_search_live` 搜索工具**：LLM 可调用的快速脚本化搜索（`platform` 参数支持淘宝），闲鱼结果支持引用回复序号收藏，适合日常询价场景
- **aiocqhttp 合并转发**：使用 aiocqhttp 渠道时，商品列表以合并转发消息发出，防止刷屏；引用转发消息回复仍可触发收藏
- **订阅操作二次确认**：LLM 在创建、删除、修改订阅前会先向用户确认，避免误操作

## 多平台支持（淘宝）

v4.0.0 起插件从「闲鱼专用」扩展为双平台：

| 能力 | 闲鱼 | 淘宝 |
| --- | --- | --- |
| 订阅监控（建/删/停/恢复/立即查） | ✅ | ✅ |
| 扫码登录 + 失效自动恢复 + 免扫码自动登录 | ✅ | ✅ |
| 实时搜索（`goofish_search_live`） | ✅ | ✅（关键词 + 价格过滤，单页） |
| 采购决策比价（`buyagent_purchase_decision`） | ✅ | ✅（双平台并发） |
| 详情深度分析 | ✅ 卖家信用 / 想要数 | ✅ 店铺 DSR / 类型 + **SKU 全档真实价目表** |
| 引用回复收藏 | ✅ | ❌（自动优雅跳过） |
| 远程 Worker 模式 | ✅ | ❌（仅本地模式） |

### 启用与使用

1. 插件配置开启 `taobao_enabled = true`（仅 `playwright_local` 本地模式）
2. 发送 `/闲鱼 登录 淘宝`，用**手机淘宝 App** 扫码；登录态独立保存（`storage_state.taobao.json`），与闲鱼互不影响
3. 之后直接说：「订阅淘宝的 RTX5090 显卡」或发送 `/闲鱼 订阅 淘宝 RTX5090 显卡`；搜商品可以说「淘宝搜一下机械键盘」，比价可以说「闲鱼淘宝比比价」——需求里点名的平台会被自动识别路由。明确的订阅句会由插件直接处理，不会落到闲聊 AI。

### 淘宝注意事项

- **列表价 ≠ 到手价**：淘宝搜索列表价是 SKU 区间最低价（低价引流），详情深度分析会给出全档 SKU 真实价目表并标注"价差引流"风险
- **风控较严**：订阅轮询间隔下限默认 30 分钟（`taobao_min_interval_sec` 可调）；浏览器必须有头运行（无桌面环境会自动拉起 Xvfb）
- 访客也能搜索，但保持登录态可显著减少滑块验证码
...
- `/闲鱼 登录 [淘宝]`：手动拉起登录流程（可选平台参数，默认闲鱼）；如果流程已经存在，会给 owner 会话刷新截图
- `/闲鱼 登录取消`：取消当前登录恢复流程（对所有平台的进行中流程生效）

说明：

- 队列在补登录态期间会暂停等待，不会继续乱跑任务。
- `登录完成` 不是必须指令；回复任意普通消息就可以继续。
- 曾登录过且浏览器 profile 里 cookie 仍有效时，发起登录会自动免扫码保存（闲鱼淘宝均支持）。
- 登录成功后，插件会优先接管当前扫码成功的活跃浏览器会话，而不是立刻关闭后重开。
- 如果你更习惯 CLI，也可以运行 `save_state.py` 手动更新本地登录态（淘宝：`save_state.py --platform taobao`）。
...
| 指令 | 作用 |
| --- | --- |
| `/闲鱼 订阅 [淘宝] <关键词> [interval_sec] [pages]` | 创建或更新订阅，并自动入队一次检查。也可直接发送「订阅淘宝的 总统黄油」 |
| `/闲鱼 退订 [淘宝] <关键词>` | 删除当前会话对应关键词订阅 |
| `/闲鱼 列表` | 查看当前会话全部订阅 |
| `/闲鱼 暂停 <关键词>` | 暂停某个订阅 |
...
| `/闲鱼 状态` | 查看调度器、Provider、队列、各平台登录态、远端健康状态 |
| `/闲鱼 登录 [淘宝]` | 手动触发补登录态（可选平台参数，默认闲鱼） |
| `/闲鱼 登录取消` | 取消当前补登录态 |

补充：

- 收藏不需要额外命令。对支持收藏的推荐消息直接"引用 + 回复序号"即可。
- 直接说「订阅淘宝的 总统黄油」或 `/闲鱼 订阅 淘宝 总统黄油` 会立刻建订阅，不经过闲聊 AI。更模糊的说法（"帮我搜一下……""给 XX 订阅降价提醒"）仍走 LLM 工具；LLM 创建/删除/修改订阅前会先向你确认。
...
- SQLite：`goofish_catcher.db`（订阅、商品、通知均带平台维度，同一关键词可双平台订阅）
- 本地登录态：闲鱼 `plugin_data/storage_state.json`；淘宝 `storage_state.taobao.json`（浏览器 profile 分别为 `browser_profile/` 与 `browser_profile_taobao/`，按平台完全隔离）
...
- 本地模式：可直接发送 `/闲鱼 登录`（淘宝订阅出问题则发送 `/闲鱼 登录 淘宝`），或重新运行 `save_state.py` 更新本地稳定登录态文件后执行 `/闲鱼 恢复 <关键词>`
...
- 闲鱼 / 淘宝页面结构变化可能导致 `PARSE_ERROR`（v3.0.0 起 Agent 降级兜底覆盖了更多此类情况）。
- 高频抓取可能触发风控，建议低并发 + 合理间隔（淘宝尤甚，间隔下限默认 30 分钟）。
- 本插件没做验证码绕过。
  - 欢迎大佬狠狠用pr灌注鼠鼠（欧尼该）
